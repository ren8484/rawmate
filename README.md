...
2. 在图库里只看 JPG。默认“挑片”导航下，按 `P` 留下想要的照片，按 `X` 或 `Delete` 标记废片；程序会自动跳到下一张未决定照片。需要逐张复查时，可在单张视图顶栏切换为“浏览”。
...
界面状态、旗标和错误日志保存在 `%LOCALAPPDATA%\RAWMate`。这些文件不包含照片本身，也不应提交到 GitHub。1.3.1 会继续读取旧数据中的 P/X 旗标，并记忆“挑片 / 浏览”导航目标；旧 `auto_advance` 设置会被安全忽略，并在下次保存设置时移除。
