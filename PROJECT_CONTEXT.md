...
注意：挑片标记绑定绝对路径；用户在程序外移动/改名照片后，旧记录可能失效。当前没有独立数据库迁移机制。
...
- 不使用顶部蓝色 banner；品牌、主题开关和操作都收敛在左侧。
- 低层级、扁平、圆角、深色为主要视觉；浅色模式必须完整可读。
