...
3. 更新三个计数和目录提示。
4. 按当前 `jpg` 文件夹中的有效照片计算 P/X 已处理进度、未处理数和独立星级计数。
...
4. `File.Move` 到对应目录。
5. 同名存在或异常则跳过，绝不覆盖。
6. 成功移动 JPG 时同步迁移 P/X/星级和当前照片状态。
...
5. 成功还原 JPG 时同步迁移 P/X/星级和当前照片状态。
...
`settings.txt` 保存目录、视图、当前照片、适应/缩放、网格尺寸、排序、筛选、自动前进开关和最近路径。主题不再保存，程序固定暗黑模式。
