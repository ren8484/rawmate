...
它不是 Lightroom 的前置工具。RAWMate 只负责决定照片留不留；挑完以后，可以继续处理 RAW，也可以直接使用 JPG。

当前正式版本：`1.2.0`（Windows 文件版本 `1.2.0.0`）。
...
2. 在图库里只看 JPG。按 `P` 留下想要的照片，按 `X` 或 `Delete` 标记废片。
3. 等“已决定”达到总数后，点击“移除所有废片”。废片 JPG 和同名 ARW 会一起进入 Windows 回收站。
...
界面状态、旗标和错误日志保存在 `%LOCALAPPDATA%\RAWMate`。这些文件不包含照片本身，也不应提交到 GitHub。1.2.0 会继续读取旧数据中的 P/X 旗标，并安全忽略已经停用的旧记录。
